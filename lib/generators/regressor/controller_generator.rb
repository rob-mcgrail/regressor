  ...
  class ControllerGenerator < Rails::Generators::Base
    ...
    def create_regression_files
      ...
      ApplicationController.descendants.map(&:name).reject { |x| Regressor.configuration.excluded_controllers.include? x }.each do |controller|
        ...
        create_file "#{Regressor.configuration.regression_controller_path}/#{controller.underscore}_routing_spec.rb", ERB.new(File.new(File.expand_path('../../templates/controller/controller_spec_template.erb', File.dirname(__FILE__))).read).result(binding)
      end
    end
